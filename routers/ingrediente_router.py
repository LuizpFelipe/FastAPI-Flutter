...
async def editar_receita(
    ...
    rendimento_padrao: Optional[float] = Form(None),
    file: Optional[UploadFile] = File(None),
    session: Session = Depends(get_session)
):
    """
    ...
    Se uma nova imagem for enviada, a anterior é deletada do servidor.
    """
...
def excluir_receita(receita_id: int, session: Session = Depends(get_session)):
    ...
    Impede a exclusão caso a receita possua vínculos em fichas técnicas ou produções ativas,
    preservando a integridade do banco de dados relacional.
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail="Não é possível excluir esta receita, pois ela possui vínculos ativos."
        )
    return {"ok": True}